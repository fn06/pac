...
let manual_debug () =
  ...
  List.iteri
    (fun i incomp ->
      ...
      let satisfied =
        incompatibility_satisfied initial_state.partial_solution incomp
      in
      let almost_satisfied =
        incompatibility_almost_satisfied initial_state.partial_solution incomp
      in

      Printf.printf "     Satisfied: %b\n" satisfied;
      Printf.printf "     Almost satisfied: %b\n" almost_satisfied;

      if satisfied then Printf.printf "     *** CONFLICT DETECTED ***\n"
      else if almost_satisfied then (
        Printf.printf "     *** UNIT PROPAGATION POSSIBLE ***\n";
        let unsatisfied_term =
          get_unsatisfied_term initial_state.partial_solution incomp
        in
        ...
        List.iteri
          (fun j term ->
            let satisfied_term =
              term_satisfied_by_partial_solution term
                initial_state.partial_solution
            in
            let contradicted_term =
              term_contradicted_by_partial_solution term
                initial_state.partial_solution
            in
            Printf.printf
              "       Term %d: %s %s - satisfied:%b contradicted:%b\n" j
              (term_package term)
              (if is_positive term then "POS" else "NEG")
              satisfied_term contradicted_term)
          incomp.terms))
    relevant_incomps;
  ...
  if not complete then (
    ...
    match make_decision repo initial_state with
