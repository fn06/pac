...
let rec debug_solve (repo : repository) (deps : dependencies) (query : query) : solve_result =
  Printf.printf "\n=== DEBUG SOLVE START ===\n";
    ...
    if iteration > max_iterations then (
      ...
      | Ok new_state ->
          ...
          if is_complete_solution new_state then (
            ...
            Ok (Solution (extract_solution new_state)))
          else (
            ...
            | Some newer_state -> (
                ...
                | Decision ((name, _), _) :: _ ->
                    Ok (debug_solve_loop newer_state name (iteration + 1))
                | _ ->
                    Printf.printf "ERROR: Invalid state after decision\n";
                    Ok (Error (InvalidInput "Invalid state after decision"))))
  in

  match query with
  | [] -> Solution []
  | (name, _) :: _ -> 
      match debug_solve_loop initial_state name 1 with
      | Ok result -> result
      | Error err -> Error err

(* Debug unit propagation with iteration tracking *)
and debug_unit_propagation state package_names iteration :
  ...
  if iteration > max_iterations + 10 then (
    ...
    | package_name :: rest -> (
        ...
        match
          debug_process_incompatibilities state relevant_incomps (iteration * 10)
        with
        | Ok new_state ->
            ...
            debug_unit_propagation new_state (rest @ changed_packages)
              (iteration + 1)
        | Error err -> Error err)

(* Debug process incompatibilities *)
and debug_process_incompatibilities state incomps iteration :
    solver_state internal_result =
  ...
  if iteration > max_iterations + 20 then (
    Printf.printf
      "      STOPPING: Process incompatibilities hit max iterations\n";
    ...
    | incomp :: rest ->
        ...
        then (
          Printf.printf
            "      Unit propagation possible for incompatibility %d\n" incomp.id;
...
and debug_is_complete_solution state =
  ...
  Printf.printf
    "    Checking completeness: %d positive derivations, %d decisions\n"
    (List.length positive_derivations)
...
let test_debug_simple () =
  ...
  | Error (NoSolution incomp) ->
      Printf.printf "\nDEBUG RESULT: No solution (incompatibility %d)\n"
        incomp.id
  | Error (InvalidInput msg) -> Printf.printf "\nDEBUG RESULT: %s\n" msg
